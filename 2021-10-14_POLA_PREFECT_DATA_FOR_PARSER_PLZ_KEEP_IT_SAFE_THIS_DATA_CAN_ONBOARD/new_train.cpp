...
class pstruct{
    ...
    private:
        ...
        inline size_t calc_dence_input_tile_number(const std::shared_ptr<pstruct> &pinfo,bool pre_node_is_Dence);
        inline size_t calc_dence_output_tile_number(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_dence_layer_tile_size(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_dence_next_layer_tile_size(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_dence_branch_next_layer_tile_size(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_dence_output_layer_tile_size(const std::shared_ptr<pstruct> &pinfo);
};
...
//----------------------FOR FULLY CONNECT(DENCE)-----------------------------
inline size_t pstruct::calc_dence_input_tile_number(const std::shared_ptr<pstruct> &pinfo,bool pre_node_is_Dence){
    size_t tmp;
    if(pre_node_is_Dence)
        tmp = pinfo->Dense_input_node;//由前一層的dense f_och * f_ox * f_oy組成
    else
        tmp = std::pow(pinfo->input_feature_size,2)*pinfo->input_channel;
    return (tmp >0) ? tmp : 1;
}
inline size_t pstruct::calc_dence_output_tile_number(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp = pinfo->Dense_output_channel * pinfo->Dense_size_output_x * pinfo->Dense_size_output_y;
    return (tmp > 0) ? tmp : 1;
}
inline size_t pstruct::calc_dence_layer_tile_size(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp = 1;
    return (MAX_TILE_SIZE > tmp) ? tmp : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_dence_next_layer_tile_size(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp = 1;
    return (MAX_TILE_SIZE > tmp) ? tmp : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_dence_branch_next_layer_tile_size(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp = 1;
    return (MAX_TILE_SIZE > tmp) ? tmp : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_dence_output_layer_tile_size(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp = 1;
    return (MAX_TILE_SIZE > tmp) ? tmp : MAX_TILE_SIZE;
}
//-----------------------------------------------------------------------
void pstruct::algorithm_for_basic_conv(const std::shared_ptr<pstruct> &merge_node,const size_t &Depth_select,const size_t &once_cal_weight_num_select,const size_t &times_calc,const size_t &OCH_NUM,const size_t &ICH_NUM, const size_t &HW_KERNEL_SIZE,const std::vector<std::vector<float>> &data_in,std::vector<short> &weight_data){
    std::ofstream ofs;
    ofs.open("./float_weight_check_info_layer_by_layer/"+merge_node->node_name+".txt");
                                ...
                                if(merge_node->kernel_size==1){
                                    if((och_count*Depth_select+och+(DATA_DEPTH*three_rd_out)) < merge_node->output_channel){ //這邊的data_depth 要改成 Depth_select 讓上面去判定你這邊應該會有多少資料 8or24
                                        if((ich_count*Depth_select+ich+(DATA_DEPTH*three_rd_in))<merge_node->input_channel){
                                                ...
                                                if(merge_node->Have_BatchNormalization){
                                                    ...
                                                    t = data_in[Weight][kernel_count] * std::pow(2,merge_node->quant_batch);
                                    ...
                                    if((och_count*Depth_select+och)<merge_node->output_channel){
                                        if((ich_count*Depth_select+ich)<merge_node->input_channel){
                                                ...
                                                t = data_in[Weight][kernel_count] * std::pow(2,merge_node->quant_batch);
                                                assert(data_in[Weight][kernel_count] * (short)t >= 0);
                                                //std::cout<<"weight "<<data_in[Weight][kernel_count] << " " << merge_node->quant_batch<<" "<<t<<" "<<kernel_count<<std::endl;
                                            #endif
                                            kernel_count++;
                                        }
                                    }
                    ...
                    if(bias<Depth_select){
                        if(bias<bias_scale_num){
                            if(merge_node->Have_BatchNormalization){
                                ...
                                #else
                                    short_tmp = (short)((data_in[Scale][och_index] / std::sqrt(data_in[Var][och_index])) * std::pow(2,merge_node->quant_batch));
                                    ofs<<"Alpha!!!!!! "<<"file_index:"<<och_index<<" Scale: "<<data_in[Scale][och_index]<<" Var: "<<data_in[Var][och_index]<<" quant_batch: "<<merge_node->quant_batch<<" ANS: "<<short_tmp<<"\n";
                        ...
                        if(bias<bias_scale_num+Depth_select){
                            if(merge_node->Have_BatchNormalization){
                                ...
                                ofs<<"Beta!!!!!! "<<"file_index:"<<och_index<<" Bias: "<<data_in[Bias][och_index]<<" Scale: "<<data_in[Scale][och_index]<<" Mean: "<<data_in[Mean][och_index]<<" Var: "<<data_in[Var][och_index]<<" ANS: "<<short_tmp<<" quant_batch: "<<std::pow(2,merge_node->quant_batch)<<"\n";
                                ...
                                short_tmp = (short)data_in[Bias][och_index] * std::pow(2,merge_node->quant_batch);
                            }else{
                                short_tmp = 0;
                            }
                        }
...
std::vector<std::shared_ptr<pstruct>>pstruct::layer_info_data_vector_trace(const std::vector<json>&layer_info_data_vector){
    ...
    for(auto layer_info : layer_info_data_vector){
        ...
        //給前面抓到自己要做分支的節點
        temp->Concat_output_control = scan_result_map[layer_info["name"]];
        ...
        for(auto target_number=0;target_number<target_layer_info_vector[target].size();target_number++){
            ...
            if(target==0&&target_number==0){
                ...
                if(Kernel_Size==3){
                    ...
                    if(layer_info_data_pointer_vector[pre_location_vec[0]]->class_name=="Conv2D"){
                        ...
                        layer_info_data_pointer_vector[now_location]->Dense_input_node = input_channel * layer_info_data_pointer_vector[now_location]->Dense_size_input_x * layer_info_data_pointer_vector[now_location]->Dense_size_input_y;
                    }
                    else if(layer_info_data_pointer_vector[pre_location_vec[0]]->class_name=="Dense"){
                        ...
                        throw std::invalid_argument("sorry，this node is Dense but pre_node is not conv or Dense，we cant support this type");
                    }
                    layer_info_data_pointer_vector[now_location]->Dense_output_node = output_channel;
                    fc_divd(layer_info_data_pointer_vector[now_location]->Dense_output_node,
                ...
                if(layer_info_data_pointer_vector[now_location]->class_name=="Conv2D"||layer_info_data_pointer_vector[now_location]->class_name=="Dense"){
                    ...
                    layer_info_data_pointer_vector[now_location]->weight_address =  max_weight_data;
                    std::cout<<"!!!!! max_weight_data "<<max_weight_data<<" weight_of_AB "<<weight_of_AB<<" weight_of_kernel "<<weight_of_kernel<<" input_channel "<<input_channel<<" output_channel "<<output_channel<<std::endl;
                    max_weight_data+=weight_of_kernel + weight_of_AB;
                }
...
void pstruct::merge_node_fix(std::vector<std::shared_ptr<pstruct>> &merge_node_vector,std::vector<int>&merge_node_jump_location,std::vector<int>&branch_node_jump_location){
    ...
    for(int i=0;i<merge_node_vector.size();i++){
        if(i==0){//first node
            ...
            merge_node_vector[i]->output_padding_size    = merge_node_vector[i]->padding=="same" ? merge_node_vector[i]->input_padding_size : 0;
            merge_node_vector[i]->input_tile_number      = calc_dence_input_tile_number(merge_node_vector[i],merge_node_vector[i-1]->class_name=="Dense");
            merge_node_vector[i]->input_tile_size        = calc_dence_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->next_tile_size         = calc_dence_next_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->branch_input_tile_size = calc_dence_branch_next_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_size       = (merge_node_vector[i]->Have_Upsample) ? merge_node_vector[i]->Upsample_size * calc_dence_output_layer_tile_size(merge_node_vector[i]) : calc_dence_output_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_number     = calc_dence_output_tile_number(merge_node_vector[i]);
...
void pstruct::gen_out_addr(std::shared_ptr<pstruct> &merge_node){
    ...
    }else if(merge_node->class_name=="Flatten"){
        auto input_tile_size = merge_node->input_tile_size;
        auto input_channel_rd = std::ceil(merge_node->input_channel/(double)Depth_select)*Depth_select;
        auto input_address = merge_node->input_address[0];
        merge_node->out_addr_set.resize(input_tile_size*input_tile_size*input_channel_rd,0);
        merge_node->out_addr_set_pool.resize(input_tile_size*input_tile_size*input_channel_rd,0);
        merge_node->valid_address_vec.resize(input_tile_size*input_tile_size*input_channel_rd,0);
        for(size_t i=0;i<input_tile_number;i++){
            for(size_t j=0;j<input_tile_number;j++){
                for(size_t k=0;k<input_channel_rd;k++){
                    merge_node->valid_address_vec[k] = input_address + k*BUS_WIDTH;
                }
            }
        }
    }else if(merge_node->class_name=="Dense"){ // fix
        auto input_tile_size = merge_node->input_tile_size;
        auto input_channel_rd = std::ceil(merge_node->input_channel/(double)24)*24;
        auto input_address = merge_node->input_address[0];
        merge_node->out_addr_set.resize(input_tile_size*input_tile_size*input_channel_rd,0);
        merge_node->out_addr_set_pool.resize(input_tile_size*input_tile_size*input_channel_rd,0);
        merge_node->valid_address_vec.resize(input_tile_size*input_tile_size*input_channel_rd,0);
...
void pstruct::get_tile_info(std::vector<std::shared_ptr<pstruct>> &merge_node_vector){
    for(auto merge_node : merge_node_vector){
        ...
        //std::cout<<i_tile_num<<std::endl;
        //std::cout<<ICH_Round<<std::endl;
        //std::cout<<OCH_Round<<std::endl;
            ...
            for(size_t j=0;j<OCH_Round;j++){//conv ok flatten fc_x fc_y och_rd but always 1
                ...
                for(const size_t layer_ich : merge_node->Previous_node_OCH){
                    ...
                    for(size_t k = 0;k<std::ceil(layer_ich/(double)Depth_select);k++){
                        ...
                        //tmp_in_addr = merge_node->input_address[input_addr_count];
                        //tmp_in_addr += k * merge_node->input_tile_number * merge_node->input_tile_number * merge_node->input_tile_size * merge_node->input_tile_size * 8 * 2;
                        //tmp_in_addr += i * merge_node->input_tile_size * merge_node->input_tile_size * 8 * 2;
                        ...
                        // if(input_addr_count==1){
                            // std::cout<<merge_node->input_address[input_addr_count]<<std::endl;
                            // std::cout<<tmp_in_addr<<std::endl;
                        // }
                        // std::cout<<tmp_in_addr<<std::endl;
...
std::string weight_type(const size_t num){
    switch(num){
        case 0:
            return "Kenerl";
        case 1:
...
std::vector<short>pstruct::gen_layer_weight(const std::shared_ptr<pstruct> &merge_node,const std::vector<std::string> &path_set){
    ...
    const size_t OCH_NUM = std::ceil(merge_node->output_channel/(double)Depth_select);
    const size_t ICH_NUM = std::ceil(merge_node->input_channel/(double)Depth_select);
    size_t once_cal_weight_num_select = merge_node->kernel_size==3 ? ONCE_CAL_WEIGHT_NUM_3 : ONCE_CAL_WEIGHT_NUM_1;
    const size_t weight_num = (OCH_NUM * ICH_NUM * once_cal_weight_num_select) + (Depth_select*2 * OCH_NUM); //你要選擇要走1*1 or 3*3
    ...
    const size_t need_file_num = merge_node->Have_BatchNormalization ? 5 : merge_node->Have_bias ? 2 : 1; //這邊之後要改因為學長這邊搞錯，應該最多有六份才對是weight的bias
    ...
    for(size_t i=0;i<need_file_num;i++){
        ...
        if(!file_set[i].is_open()){
            std::cout<<fmt::format("Can not open {} {} file\n",merge_node->weight_name,weight_type(i))<<std::endl;
    ...
    size_t times_calc = merge_node->kernel_size==1 ? 3 : 1; //如果你的kernel_size是3*3的話那你就不用跑三遍，因為你是8 in 8 out，但是如果你是1*1 那你就要一個ch 24
    ...
    for(size_t i=0;i<path_num;i++){
        switch (i){
            ...
            case Bias:
                ...
                break;
            case Scale:
                tmp = merge_node->weight_name + "_Scale.txt";
                break;
            case Var:
...
int main(int argc, char **argv){
    ...
    //DUMP_LAYER INFO
        ...
        std::vector<std::vector<short>> total_weight = pola.gen_total_weight(argv[3],pola.merge_node_vector);
            ...
            for(auto i =0;i<total_weight.size();i++){
                ...
                std::cout<<total_weight_split[0]<<std::endl;
    ...
    for(std::shared_ptr<pstruct> &merge_node : pola.merge_node_vector){
            ...
            for(auto i:output_layers){
                if(merge_node->weight_name==i[0]){
                    out<<fmt::format("{}\n",merge_node->tile_info[0]);
                }
            }
        #endif
    }
