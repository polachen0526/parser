...
struct File_Type_define{
    ...
    size_t variance;
    size_t mean;
    size_t lstm_weight_x;
    size_t lstm_weight_h;
    size_t lstm_bias;
} File_Attributes;
class pstruct{
    public:
        ...
        void scan_table(const json layers,const std::string output_layer); //for concat you need take out the address and concat_control
        ...
        void lstm_dicd(int32_t lstm_input , int32_t &dimx,int32_t &dimy);
        //----------------------parameter----------------
        int32_t input_feature_size_x                = -1;
        int32_t input_feature_size_y                = -1;
        int32_t output_feature_size_x               = -1;
        int32_t output_feature_size_y               = -1;
        int32_t input_channel                       = -1;
        int32_t output_channel                      = -1;
        int32_t kernel_size                         = -1;
        int32_t kernel_stride_x                     = -1;
        int32_t kernel_stride_y                     = -1;
        int32_t pool_size_x                         = -1;
        int32_t pool_size_y                         = -1;
        int32_t pool_stride_x                       = -1;
        int32_t pool_stride_y                       = -1;
        ...
        int32_t input_tile_size_x                   = -1;
        int32_t input_tile_size_y                   = -1;
        int32_t input_tile_number_dense             = -1;
        int32_t input_tile_number_x                 = -1;
        int32_t input_tile_number_y                 = -1;
        int32_t output_tile_size_x                  = -1;
        int32_t output_tile_size_y                  = -1;
        int32_t output_tile_number_x                = -1;
        int32_t output_tile_number_y                = -1;
        int32_t output_tile_number_dense            = -1;
        int32_t next_tile_size_x                    = -1;
        int32_t next_tile_size_y                    = -1;
        int32_t branch_input_tile_size_x            = -1; //現在看他和Input_tile_size不是一樣ㄇ，但是如果你padding那邊不是same的話，如果今天是valid那你這邊就會和input_tile_size差看看幾個CONV，因為你沒有PADDING，這邊是從output_feature_size回推
        int32_t branch_input_tile_size_y            = -1;
        int32_t output_address                      = -1;
        int32_t pool_address                        = 0;
        int32_t Bit_Serial                          = 0;
        int32_t quant_batch_bias_alpha              = 0;
        int32_t quant_batch_bias_beta               = 0;
        ...
        bool IF_PRE_NODE_IS_LSTM     = false;
        ...
        void algorithm_for_LSTM(const std::shared_ptr<pstruct> &merge_node,const std::vector<std::vector<float>> &data_in,std::vector<short> &weight_data);
    private:
        size_t round_ch_8(const size_t ch);
        size_t round_ch_24(const size_t ch);
        inline size_t calc_input_tile_number_x(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_input_tile_number_y(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_output_tile_number_x(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_output_tile_number_y(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_next_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_next_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_branch_next_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_branch_next_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_output_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo);
        inline size_t calc_output_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo);
        //func for dense
...
//----------------------FOR CONV SERIES-----------------------------
inline size_t pstruct::calc_input_tile_number_x(const std::shared_ptr<pstruct> &pinfo){
    const size_t tile_size = MAX_TILE_SIZE;//- pinfo->input_padding_size*2;
    const size_t tmp = std::ceil(pinfo->input_feature_size_x/(double)tile_size);
    return (tmp >0) ? tmp : 1;
}
inline size_t pstruct::calc_input_tile_number_y(const std::shared_ptr<pstruct> &pinfo){
    const size_t tile_size = MAX_TILE_SIZE;//- pinfo->input_padding_size*2;
    const size_t tmp = std::ceil(pinfo->input_feature_size_y/(double)tile_size);
    return (tmp >0) ? tmp : 1;
}
inline size_t pstruct::calc_output_tile_number_x(const std::shared_ptr<pstruct> &pinfo){
    const size_t tile_size = MAX_TILE_SIZE;//- pinfo->output_padding_size*2;
    const size_t tmp = std::ceil(pinfo->output_feature_size_x/(double)tile_size);
    return (tmp > 0) ? tmp : 1;
}
inline size_t pstruct::calc_output_tile_number_y(const std::shared_ptr<pstruct> &pinfo){
    const size_t tile_size = MAX_TILE_SIZE;//- pinfo->output_padding_size*2;
    const size_t tmp = std::ceil(pinfo->output_feature_size_y/(double)tile_size);
    return (tmp > 0) ? tmp : 1;
}
inline size_t pstruct::calc_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp_x = pinfo->input_feature_size_x;// + pinfo->input_padding_size*2;
    return (MAX_TILE_SIZE > tmp_x) ? tmp_x : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp_y = pinfo->input_feature_size_y;// + pinfo->input_padding_size*2;
    return (MAX_TILE_SIZE > tmp_y) ? tmp_y : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_next_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp_x = pinfo->output_feature_size_x;// + pinfo->output_padding_size*2;
    return (MAX_TILE_SIZE > tmp_x) ? tmp_x : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_next_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp_y = pinfo->output_feature_size_y;// + pinfo->output_padding_size*2;
    return (MAX_TILE_SIZE > tmp_y) ? tmp_y : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_branch_next_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp_x = (pinfo->Have_Maxpooling) ? (pinfo->output_feature_size_x * pinfo->pool_size_x) : (pinfo->output_feature_size_x);
    return (MAX_TILE_SIZE > tmp_x) ? tmp_x : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_branch_next_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo){
    const size_t tmp_y = (pinfo->Have_Maxpooling) ? (pinfo->output_feature_size_y * pinfo->pool_size_y) : (pinfo->output_feature_size_y);
    return (MAX_TILE_SIZE > tmp_y) ? tmp_y : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_output_layer_tile_size_x(const std::shared_ptr<pstruct> &pinfo){
    size_t tmp_x = pinfo->input_tile_size_x /  (pinfo->kernel_stride_x>0 ? pinfo->kernel_stride_x : 1); 
    if(pinfo->pool_stride_x>0)
        tmp_x = tmp_x/pinfo->pool_stride_x;
    return (MAX_TILE_SIZE > tmp_x) ? tmp_x : MAX_TILE_SIZE;
}
inline size_t pstruct::calc_output_layer_tile_size_y(const std::shared_ptr<pstruct> &pinfo){
    size_t tmp_y = pinfo->input_tile_size_y /  (pinfo->kernel_stride_y>0 ? pinfo->kernel_stride_y : 1); 
    if(pinfo->pool_stride_y>0)
        tmp_y = tmp_y/pinfo->pool_stride_y;
    return (MAX_TILE_SIZE > tmp_y) ? tmp_y : MAX_TILE_SIZE;
}
...
inline size_t pstruct::calc_dense_input_tile_number(const std::shared_ptr<pstruct> &pinfo){
    ...
    if(pinfo->IF_PRE_NODE_IS_DENSE)
        ...
        tmp = pinfo->input_feature_size_x*pinfo->input_feature_size_y * round_ch_24(pinfo->input_channel)*DATA_Depth_24;
    return (tmp >0) ? tmp : 1;
}
                    ...
                    for(size_t three_rd_out=0;three_rd_out<times_calc;three_rd_out++){
                        for(size_t och=0;och<DATA_DEPTH;och++){
                            ...
                            if ( alpha_beta == 0 ){ // This is 1 
                                t = 1 * std::pow(2,merge_node->quant_batch_bias_alpha);
                                weight_data[index] = t;
                            } else if ( alpha_beta == 1 && o_index < merge_node->Dense_output_node && och_index < merge_node->Dense_output_channel ) { 
                                t = data_in[File_Attributes.bias][kernel_count] * std::pow(2,merge_node->quant_batch_bias_beta);
                                weight_data[index] = t;
                                kernel_count ++;
                            }
                ...
                for(size_t ich_count = 0; ich_count<ICH_NUM; ich_count++){
                    for(size_t three_rd_in = 0; three_rd_in<times_calc;three_rd_in++){
                        for(size_t ich=0;ich<DATA_DEPTH;ich++){
                            for(size_t k=0;k<HW_KERNEL_SIZE;k++){//2021/8/28 這邊的KERNEL_SIZE原本是設定9讓不管是1*1 OR 3*3都是用9去做
                                short t=0;
                    ...
                    if(bias<Depth_select){
                        if(bias<bias_scale_num){
                            if(merge_node->Have_BatchNormalization){
                                ...
                                #else
                                    auto before_short_tmp = (data_in[File_Attributes.gamma][och_index] / std::sqrt(data_in[File_Attributes.variance][och_index]));
                                    before_short_tmp = before_short_tmp > 1 ? 1 : before_short_tmp < -1 ? -1 : before_short_tmp;
                                    short_tmp = (short)(before_short_tmp * std::pow(2,merge_node->quant_batch_bias_alpha));
                                    ofs<<"Alpha!!!!!! "<<"file_index:"<<och_index<<" Scale: "<<data_in[File_Attributes.gamma][och_index]<<" Var: "<<data_in[File_Attributes.variance][och_index]<<" quant_batch: "<<merge_node->quant_batch_bias_alpha<<" ANS: "<<short_tmp<<"\n";
                                    if(data_in[File_Attributes.gamma][och_index] / std::sqrt(data_in[File_Attributes.variance][och_index]) * (short)short_tmp < 0){
                                        std::cout << fmt::format("Scale {}, Var {}, Fix {}, Float {}\n", data_in[File_Attributes.gamma][och_index],
                                        ...
                                        short_tmp = before_short_tmp>0 ? 32767 : -32768; // if the value will be overflow then the value is max value 
                                    }
                                    //std::cout<<before_short_tmp<<"        &&       "<<short_tmp<<std::endl;
                                    assert(before_short_tmp * short_tmp >= 0);
                                #endif
                            }else if(merge_node->Have_bias){
                                short_tmp = std::pow(2,merge_node->quant_batch_bias_alpha);
                                auto value = index+bias;
                                ofs<<"Alpha!!!!!"<<short_tmp<<" "<<value<<"\n";
                            }else{
                                short_tmp = 0;
                                ofs<<"Alpha!!!!!"<<short_tmp<<"\n";
                            }
                        ...
                        if(bias<bias_scale_num+Depth_select){
                            if(merge_node->Have_BatchNormalization){
                                auto var = data_in[File_Attributes.variance][och_index]==0 ? 0.000000000000000000000000000001 : data_in[File_Attributes.variance][och_index];
                                auto new_bias = data_in[File_Attributes.beta][och_index] - (data_in[File_Attributes.gamma][och_index] * data_in[File_Attributes.mean][och_index] / std::sqrt(var));
                                new_bias = new_bias > 1 ? 1 : (new_bias<-1) ? -1 : new_bias;
                                short_tmp = (short)((new_bias) * std::pow(2,merge_node->quant_batch_bias_beta));
                                ofs<<"Beta!!!!!! "<<"file_index:"<<och_index<<" Bias: "<<data_in[File_Attributes.beta][och_index]<<" Scale: "<<data_in[File_Attributes.gamma][och_index]<<" Mean: "<<data_in[File_Attributes.mean][och_index]<<" Var: "<<data_in[File_Attributes.variance][och_index]<<" ANS: "<<short_tmp<<" quant_batch: "<<merge_node->quant_batch_bias_beta<<"\n";
                                //std::cout<<short_tmp<<"&&"<<new_bias<<std::endl;
                                assert(new_bias * (short)short_tmp >= 0);
                            }else if(merge_node->Have_bias){
                                //std::cout<<data_in[File_Attributes.bias][och_index]<<std::endl;
                                short_tmp = (short)(data_in[File_Attributes.bias][och_index] * std::pow(2,merge_node->quant_batch_bias_beta));
                                assert(data_in[File_Attributes.bias][och_index] * short_tmp >= 0);
                                auto value = index+bias;
                                ofs<<"Beta!!!!!"<<short_tmp<<" "<<value<<"\n";
                            }else{
                                short_tmp = 0;
                                ofs<<"Beta!!!!!"<<short_tmp<<"\n";
                            }
...
void pstruct::algorithm_for_LSTM(const std::shared_ptr<pstruct> &merge_node,const std::vector<std::vector<float>> &data_in,std::vector<short> &weight_data){
    std::ofstream ofs;
    ofs.open(merge_node->node_name+".txt");
    std::vector<std::vector<float>> weight_data_2d_x;
    std::vector<std::vector<float>> weight_data_2d_h;
    std::vector<float> lstm_bias;
    size_t weight_x = 0;
    size_t weight_h = 1;
    size_t bias = 2;
    weight_data_2d_x.resize(merge_node->input_channel);
    weight_data_2d_h.resize(merge_node->output_channel);
    lstm_bias.resize(merge_node->output_channel*4);
    for(int i=0;i<merge_node->input_channel;i++){
        for(int j=0;j<merge_node->output_channel*4;j++){
            weight_data_2d_x[i].push_back(data_in[weight_x][i*merge_node->output_channel*4+j]);
            //std::cout<<data_in[weight_x][i*merge_node->input_channel+j]<<std::endl;
        }
    }
    for(int i=0;i<merge_node->output_channel;i++){
        for(int j=0;j<merge_node->output_channel*4;j++){
            weight_data_2d_h[i].push_back(data_in[weight_h][i*merge_node->output_channel*4+j]);
            //std::cout<<data_in[weight_h][i*merge_node->input_channel+j]<<std::endl;
        }
    }
    for(int i=0;i<merge_node->output_channel*4;i++){
        lstm_bias[i] = data_in[bias][i];
        //std::cout<<lstm_bias[i]<<std::endl;
    }
    std::cout<<weight_data_2d_x.size()<<std::endl;
    std::cout<<weight_data_2d_h.size()<<std::endl;
    std::cout<<lstm_bias.size()<<std::endl;
    //----------------------start do the memory sort-----------------------
    size_t per_round = 8;
    size_t gates = 4;
    size_t index = 0;
    size_t borader = merge_node->input_channel;
    size_t total_ronud_x = (merge_node->output_channel)/per_round; //128
    size_t total_ronud_y = (merge_node->input_channel+merge_node->output_channel); //640
    //std::cout<<weight_data_2d_x[512][0]<<std::endl;
    for(auto round_x = 0;round_x<total_ronud_x;round_x++){
        for(auto gate = 0;gate < gates;gate++){
            for(auto round_y = 0;round_y < total_ronud_y;round_y++){
                for(auto per_round_count=0;per_round_count<per_round;per_round_count++){
                    if(round_y<borader){
                        //std::cout<<weight_data_2d_x[round_y][per_round_count + gate * merge_node->output_channel + round_x*per_round]<<std::endl;
                        weight_data[index] = (short)(weight_data_2d_x[round_y][per_round_count + gate * merge_node->output_channel + round_x*per_round] * std::pow(2,merge_node->quant_batch));
                    }
                    else{
                        //std::cout<<weight_data_2d_h[round_y][per_round_count + gate * merge_node->output_channel + round_x*per_round]<<std::endl;
                        weight_data[index] = (short)(weight_data_2d_h[round_y-borader][per_round_count + gate * merge_node->output_channel + round_x*per_round] * std::pow(2,merge_node->quant_batch));
                    }
                    ofs<<round_x<<" "<<gate<<" "<<round_y<<" "<<per_round_count + gate * merge_node->output_channel + round_x*per_round<<" value: "<<weight_data[index]<<"\n";
                    index++;
                }
            }
            for(auto bias_count=0;bias_count<per_round;bias_count++){
                auto trams_bias = short(lstm_bias[bias_count + gate * merge_node->output_channel + round_x*per_round] * std::pow(2,merge_node->quant_batch_bias_beta));
                weight_data[index] = trams_bias;
                ofs<<bias_count<<" value: "<<weight_data[index]<<"\n";
                index++;
            }
        }
    }
...
void pstruct::lstm_dicd(int32_t lstm_input , int32_t &dimx,int32_t &dimy){
    std::vector<int32_t> compare_answer;
    for(int tmp=2;tmp<=32;tmp++){
        if(lstm_input%tmp==0){
            if(tmp<dimx){
                dimx = lstm_input/tmp;
                dimy = tmp;
...
std::vector<std::shared_ptr<pstruct>>pstruct::layer_info_data_vector_trace(const std::vector<json>&layer_info_data_vector){
    ...
    for(auto layer_info : layer_info_data_vector){
        ...
        if(layer_info["class_name"]=="InputLayer"){
            ...
            temp->input_feature_size_x  = (!layer_info["config"]["batch_input_shape"][2].empty()) ? layer_info["config"]["batch_input_shape"][2]   : (json)n_none;
            temp->input_feature_size_y  = (!layer_info["config"]["batch_input_shape"][1].empty()) ? layer_info["config"]["batch_input_shape"][1]   : (json)n_none;
        ...
        if(layer_info["class_name"]=="Conv2D"){
            ...
            temp->kernel_stride_x     = (!layer_info["config"]["strides"][1].empty())           ? layer_info["config"]["strides"][1]           : (json)n_none; 
            temp->kernel_stride_y     = (!layer_info["config"]["strides"][0].empty())           ? layer_info["config"]["strides"][0]           : (json)n_none; 
            ...
            temp->units               = n_none; 
            temp->pool_size_x         = n_none;
            temp->pool_size_x         = n_none;
            temp->pool_padding        = s_none; //s
            temp->pool_stride_x       = n_none;
            temp->pool_stride_y       = n_none;
            temp->node_name           = layer_info["config"]["name"];
            temp->Have_ReLU           = temp->activate=="relu" ? true : false;
        }
        if(layer_info["class_name"]=="MaxPooling2D"){
            temp->class_name          = (!layer_info["class_name"].empty())                   ? layer_info["class_name"]                     : (json)s_none; //s
            temp->pool_size_x         = (!layer_info["config"]["pool_size"][1].empty())       ? layer_info["config"]["pool_size"][1]         : (json)n_none;
            temp->pool_size_y         = (!layer_info["config"]["pool_size"][0].empty())       ? layer_info["config"]["pool_size"][0]         : (json)n_none;
            temp->pool_padding        = (!layer_info["config"]["padding"].empty())            ? layer_info["config"]["padding"]              : (json)s_none; //s
            temp->pool_stride_x       = (!layer_info["config"]["strides"][1].empty())         ? layer_info["config"]["strides"][1]           : (json)n_none;
            temp->pool_stride_y       = (!layer_info["config"]["strides"][0].empty())         ? layer_info["config"]["strides"][0]           : (json)n_none;
            temp->input_feature_size_x  = n_none;
            temp->input_feature_size_y  = n_none;
            temp->input_channel       = n_none;
            temp->output_channel      = n_none; 
            temp->kernel_size         = n_none; 
            temp->kernel_stride_x     = n_none; 
            temp->kernel_stride_y     = n_none; 
        ...
        if(layer_info["class_name"]=="Lambda"){
            temp->Have_Lambda             = true;
            temp->class_name              = layer_info["class_name"];
            temp->node_name               = layer_info["config"]["name"];
            temp->Lambda_func             = "squeeze";
        }
        //給前面抓到自己要做分支的節點
        temp->Concat_output_control = (scan_result_map.size()!=0) ? scan_result_map[layer_info["name"]] : 0; //scan_result_map leakyrelu8 leakyrelu5
        ...
        for(auto i : concat_map_vector){ //up_sampling2d_1 leaky_re_lu_5 
        ...
        //給後面去需要前面資訊的節點
        for(auto i : scan_result_branch_node_map){ //conv2d_11 concatenate_1
            if(layer_info["name"]==i.first){
                temp->branch_node = 1;
                if(!temp->Have_Concat){
                ...
                for(auto node : temp->branch_node_location){
                    std::cout<<temp->node_name<<" "<<node<<std::endl;
                }
            }
        }
        ...
        for(auto target_number=0;target_number<target_layer_info_vector[target].size();target_number++){
            ...
            if(target==0&&target_number==0){
                if( layer_info_data_pointer_vector[now_location]->class_name=="InputLayer"){
                    ...
                    layer_info_data_pointer_vector[now_location]->output_feature_size_x = layer_info_data_pointer_vector[now_location]->input_feature_size_x;
                    layer_info_data_pointer_vector[now_location]->output_feature_size_y = layer_info_data_pointer_vector[now_location]->input_feature_size_y;
                    continue;
                }
                ...
                int32_t ifs_x = (layer_info_data_pointer_vector[now_location]->input_feature_size_x==-1) ? throw std::invalid_argument("the input layer ifs is negative") : layer_info_data_pointer_vector[now_location]->input_feature_size_x;
                int32_t ifs_y = (layer_info_data_pointer_vector[now_location]->input_feature_size_y==-1) ? throw std::invalid_argument("the input layer ifs is negative") : layer_info_data_pointer_vector[now_location]->input_feature_size_y;
                int32_t ofs_x = 0;
                int32_t ofs_y = 0;
                if(layer_info_data_pointer_vector[now_location]->class_name=="Conv2D"){
                    if(layer_info_data_pointer_vector[now_location]->padding=="valid"){
                        ...
                        if(Kernel_Size==3){
                            ofs_x = (ifs_x-layer_info_data_pointer_vector[now_location]->kernel_size+1)/layer_info_data_pointer_vector[now_location]->kernel_stride_x;
                            ofs_y = (ifs_y-layer_info_data_pointer_vector[now_location]->kernel_size+1)/layer_info_data_pointer_vector[now_location]->kernel_stride_y;
                        }else if(Kernel_Size==2){
                            ofs_x = (ifs_x-layer_info_data_pointer_vector[now_location]->kernel_size+1)/layer_info_data_pointer_vector[now_location]->kernel_stride_x;
                            ofs_y = (ifs_y-layer_info_data_pointer_vector[now_location]->kernel_size+1)/layer_info_data_pointer_vector[now_location]->kernel_stride_y;
                        }else if(Kernel_Size==1){
                            ofs_x = (ifs_x)/layer_info_data_pointer_vector[now_location]->kernel_stride_x;
                            ofs_y = (ifs_y)/layer_info_data_pointer_vector[now_location]->kernel_stride_y;
                        }else{
                            throw std::invalid_argument("error we dont have this padding valid code 797 option");
                        }
                        ...
                        size_t tmp_of_size_x,tmp_of_size_y;
                        ofs_x = ifs_x/layer_info_data_pointer_vector[now_location]->kernel_stride_x;
                        ofs_y = ifs_y/layer_info_data_pointer_vector[now_location]->kernel_stride_y;
                        ...
                        if(Kernel_Size==3){
                            tmp_of_size_x = (size_t)std::ceil((ifs_x-layer_info_data_pointer_vector[now_location]->kernel_size+1)/(double)layer_info_data_pointer_vector[now_location]->kernel_stride_x);
                            tmp_of_size_y = (size_t)std::ceil((ifs_y-layer_info_data_pointer_vector[now_location]->kernel_size+1)/(double)layer_info_data_pointer_vector[now_location]->kernel_stride_y);
                        }else if(Kernel_Size==2){
                            tmp_of_size_x = (size_t)std::ceil((ifs_x-layer_info_data_pointer_vector[now_location]->kernel_size+1)/(double)layer_info_data_pointer_vector[now_location]->kernel_stride_x);
                            tmp_of_size_y = (size_t)std::ceil((ifs_y-layer_info_data_pointer_vector[now_location]->kernel_size+1)/(double)layer_info_data_pointer_vector[now_location]->kernel_stride_y);
                        }else if(Kernel_Size==1){
                            tmp_of_size_x = (size_t)std::ceil((ifs_x)/(double)layer_info_data_pointer_vector[now_location]->kernel_stride_x);
                            tmp_of_size_y = (size_t)std::ceil((ifs_y)/(double)layer_info_data_pointer_vector[now_location]->kernel_stride_y);
                        }else{
                            throw std::invalid_argument("error we dont have this padding same code 812 option");
                        }
                        layer_info_data_pointer_vector[now_location]->input_padding_size = (ofs_x-tmp_of_size_x)/2;
                    }
                ...
                if(Kernel_Size==3){
                    ...
                    weight_of_AB     = layer_info_data_pointer_vector[now_location]->Have_bias ? round_ch_8(output_channel)*DATA_DEPTH*2*DATA_WIDTH //alpha beta bias
                                                                                              : round_ch_8(output_channel)*DATA_DEPTH*2*DATA_WIDTH;//alpha beta
                }else if(Kernel_Size==2){//我給位置的時候如果是2我需要先給3因為我是用3*3支援2*2使用率4/9
                    weight_of_kernel = round_ch_8(input_channel)*round_ch_8(output_channel)*std::pow(/*Kernel_Size*/3,2)*DATA_DEPTH*DATA_DEPTH*DATA_WIDTH;
                    weight_of_AB     = layer_info_data_pointer_vector[now_location]->Have_bias ? round_ch_8(output_channel)*DATA_DEPTH*2*DATA_WIDTH //alpha beta bias
                                                                                              : round_ch_8(output_channel)*DATA_DEPTH*2*DATA_WIDTH;//alpha beta
                }else if(Kernel_Size==1){
                ...
                layer_info_data_pointer_vector[now_location]->input_feature_size_x  = ifs_x;
                layer_info_data_pointer_vector[now_location]->input_feature_size_y  = ifs_y;
                layer_info_data_pointer_vector[now_location]->output_feature_size_x = ofs_x;
                layer_info_data_pointer_vector[now_location]->output_feature_size_y = ofs_y;
            }else{
                ...
                if(!layer_info_data_pointer_vector[now_location]->branch_node){
                    ...
                    if(layer_info_data_pointer_vector[now_location]->Have_Concat){
                        for(auto index : layer_info_data_pointer_vector[now_location]->branch_node_location){
                            ...
                            std::cout<<"this node is "<< layer_info_data_pointer_vector[now_location]->node_name<<"concat and node location value is "<<index<<std::endl;
                        }
                    }
                    else{
                        std::cout<<layer_info_data_pointer_vector[now_location]->node_name<<std::endl;
                    ...
                    for(auto index : pre_location_vec){
                        std::cout<<"branch_node got pre_location_vec is "<<index<<std::endl;
                    }
                }
                //const int32_t Kernel_Size   = (3>layer_info_data_pointer_vector[now_location]->kernel_size) ?  3 : layer_info_data_pointer_vector[now_location]->kernel_size;
                ...
                int32_t ifs_x = layer_info_data_pointer_vector[now_location]->input_feature_size_x==-1 ? layer_info_data_pointer_vector[pre_location_vec[0]]->output_feature_size_x : layer_info_data_pointer_vector[now_location]->input_feature_size_x;
                int32_t ifs_y = layer_info_data_pointer_vector[now_location]->input_feature_size_y==-1 ? layer_info_data_pointer_vector[pre_location_vec[0]]->output_feature_size_y : layer_info_data_pointer_vector[now_location]->input_feature_size_y;
                int32_t ofs_x = 0;
                int32_t ofs_y = 0;
                ...
                else if(layer_info_data_pointer_vector[now_location]->class_name=="LSTM"){
                    output_channel = layer_info_data_pointer_vector[now_location]->units;
                    layer_info_data_pointer_vector[now_location]->Have_LSTM = true;
                }
                ...
                if(layer_info_data_pointer_vector[now_location]->class_name=="Conv2D"){
                    if(layer_info_data_pointer_vector[now_location]->padding=="valid"){
                        ...
                        if(Kernel_Size==3){
                            ...
                            throw std::invalid_argument("sorry you need to check code 939 you got the error Kernel size");
                        }
                    }
                    else if(layer_info_data_pointer_vector[now_location]->padding=="same"){
                        size_t tmp_of_size_x , tmp_of_size_y;
                        ...
                        if(Kernel_Size==3){
                            ...
                            throw std::invalid_argument("sorry u got the error Kernel_Size check code 957");
                        }
                        layer_info_data_pointer_vector[now_location]->input_padding_size = (ofs_x-tmp_of_size_x)/2;
                    }
                    else if(layer_info_data_pointer_vector[now_location]->padding=="none"){
                    ...
                    if(layer_info_data_pointer_vector[now_location]->pool_padding=="valid"){
                        ofs_x = std::ceil((ifs_x-layer_info_data_pointer_vector[now_location]->pool_size_x+1)/(double)layer_info_data_pointer_vector[now_location]->pool_stride_x);
                        ofs_y = std::ceil((ifs_y-layer_info_data_pointer_vector[now_location]->pool_size_y+1)/(double)layer_info_data_pointer_vector[now_location]->pool_stride_y);
                    }
                    else if(layer_info_data_pointer_vector[now_location]->pool_padding=="same"){
                        ofs_x = ifs_x/layer_info_data_pointer_vector[now_location]->pool_stride_x;
                        ofs_y = ifs_y/layer_info_data_pointer_vector[now_location]->pool_stride_y;
                    }
                    ...
                    //ofs_x = 1; //FC出來的output_feature_size都是1
                    //ofs_y = 1; //FC出來的output_feature_size都是1
                    ofs_x = layer_info_data_pointer_vector[pre_location]->class_name=="Flatten" ? 1 : ifs_x;
                    ofs_y = layer_info_data_pointer_vector[pre_location]->class_name=="Flatten" ? 1 : ifs_y;
                }else{
                    ofs_x = ifs_x;
                    ofs_y = ifs_y;
                }
                ...
                if(layer_info_data_pointer_vector[now_location]->class_name=="Dense"){
                    if(layer_info_data_pointer_vector[pre_location_vec[0]]->class_name=="Dense"){
                        ...
                        layer_info_data_pointer_vector[now_location]->Dense_input_node = layer_info_data_pointer_vector[now_location]->Dense_input_channel;//lstm only channel for node
                    }
                    else{
                        layer_info_data_pointer_vector[now_location]->IF_PRE_NODE_IS_DENSE = false;
                        layer_info_data_pointer_vector[now_location]->IF_PRE_NODE_IS_LSTM = false;
                        layer_info_data_pointer_vector[now_location]->Dense_size_input_x = (MAX_TILE_SIZE > ifs_x) ? ifs_x : MAX_TILE_SIZE;
                        layer_info_data_pointer_vector[now_location]->Dense_size_input_y = (MAX_TILE_SIZE > ifs_y) ? ifs_y : MAX_TILE_SIZE;
                        layer_info_data_pointer_vector[now_location]->Dense_input_channel = input_channel;
                    ...
                    weight_of_kernel =  round_ch_8(output_channel) * (input_channel + output_channel) * DATA_DEPTH * 4 * DATA_WIDTH;//4 gate
                    weight_of_AB     =  round_ch_8(output_channel) * DATA_DEPTH * 4 * DATA_WIDTH;
                }else if(Kernel_Size==3){
                    weight_of_kernel = round_ch_8(input_channel)*round_ch_8(output_channel)*std::pow(Kernel_Size,2)*DATA_DEPTH*DATA_DEPTH*DATA_WIDTH;
                    weight_of_AB     = round_ch_8(output_channel)*DATA_DEPTH*2*DATA_WIDTH;
                }else if(Kernel_Size==2){
                    weight_of_kernel = round_ch_8(input_channel)*round_ch_8(output_channel)*std::pow(/*Kernel_Size*/3,2)*DATA_DEPTH*DATA_DEPTH*DATA_WIDTH;//for OCR model
                    weight_of_AB     = round_ch_8(output_channel)*DATA_DEPTH*2*DATA_WIDTH;
                }else if(Kernel_Size==1){
                    weight_of_kernel = round_ch_24(input_channel)*round_ch_24(output_channel)*std::pow(Kernel_Size,2)*DATA_Depth_24*DATA_Depth_24*DATA_WIDTH;
                ...
                if(layer_info_data_pointer_vector[now_location]->class_name=="Conv2D"||layer_info_data_pointer_vector[now_location]->class_name=="Dense"||layer_info_data_pointer_vector[now_location]->class_name=="LSTM"){
                    for(int  target=0;target<target_layer_info_vector.size();target++){
                        for(auto target_number=0;target_number<target_layer_info_vector[target].size();target_number++){
...
std::vector<std::shared_ptr<pstruct>>pstruct::merge_node(std::vector<std::shared_ptr<pstruct>> &layer_info_data_pointer_vector,std::vector<std::vector<int>> &target_layer_info_vector,std::vector<std::vector<int>> &target_layer_quant_vector,std::vector<int>&merge_node_jump_location){
    ...
    for(int  target=0;target<target_layer_info_vector.size();target++){
        ...
        for(auto target_number=0;target_number<target_layer_info_vector[target].size();target_number++){
            new_pointer->node_name              = (new_pointer->node_name=="none")                    ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->node_name               : new_pointer->node_name;
            ...
            new_pointer->input_feature_size_x   = new_pointer->input_feature_size_x==-1               ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->input_feature_size_x      : new_pointer->input_feature_size_x;
            new_pointer->input_feature_size_y   = new_pointer->input_feature_size_y==-1               ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->input_feature_size_y      : new_pointer->input_feature_size_y;
            ...
            new_pointer->output_feature_size_x  = layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->output_feature_size_x;//your output_feature size will be replace when the next node
            new_pointer->output_feature_size_y  = layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->output_feature_size_y;//your output_feature size will be replace when the next node
            ...
            new_pointer->kernel_stride_x        = (new_pointer->kernel_stride_x==-1)                ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->kernel_stride_x         : new_pointer->kernel_stride_x;
            new_pointer->kernel_stride_y        = (new_pointer->kernel_stride_y==-1)                ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->kernel_stride_y         : new_pointer->kernel_stride_y;
            ...
            new_pointer->pool_size_x            = (new_pointer->pool_size_x==-1)                    ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->pool_size_x             : new_pointer->pool_size_x;
            new_pointer->pool_size_y            = (new_pointer->pool_size_y==-1)                    ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->pool_size_y             : new_pointer->pool_size_y;
            new_pointer->pool_stride_x          = (new_pointer->pool_stride_x==-1)                  ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->pool_stride_x           : new_pointer->pool_stride_x;
            new_pointer->pool_stride_y          = (new_pointer->pool_stride_y==-1)                  ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->pool_stride_y           : new_pointer->pool_stride_y;
            ...
            new_pointer->Have_LSTM            = (new_pointer->Have_LSTM==false)                   ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->Have_LSTM               : new_pointer->Have_LSTM;
            ...
            new_pointer->IF_PRE_NODE_IS_LSTM  = (new_pointer->IF_PRE_NODE_IS_LSTM==false)         ? layer_info_data_pointer_vector[target_layer_info_vector[target][target_number]]->IF_PRE_NODE_IS_LSTM     : new_pointer->IF_PRE_NODE_IS_LSTM;
        ...
        //----------------------上面兩個差非常多喔~~~~----------------------------
        new_pointer->output_feature_size_x = (new_pointer->Have_Upsample) ? new_pointer->output_feature_size_x*2 : new_pointer->output_feature_size_x;
        new_pointer->output_feature_size_y = (new_pointer->Have_Upsample) ? new_pointer->output_feature_size_y*2 : new_pointer->output_feature_size_y;
...
void pstruct::merge_node_fix(std::vector<std::shared_ptr<pstruct>> &merge_node_vector,std::vector<int>&merge_node_jump_location,std::vector<int>&branch_node_jump_location){
    ...
    for(int i=0;i<merge_node_vector.size();i++){
        if(i==0){//first node
            ...
            merge_node_vector[i]->input_tile_number_x    = calc_input_tile_number_x(merge_node_vector[i]);
            merge_node_vector[i]->input_tile_number_y    = calc_input_tile_number_y(merge_node_vector[i]);
            merge_node_vector[i]->input_tile_size_x      = calc_layer_tile_size_x(merge_node_vector[i]);
            merge_node_vector[i]->input_tile_size_y      = calc_layer_tile_size_y(merge_node_vector[i]);
            merge_node_vector[i]->next_tile_size_x       = calc_next_layer_tile_size_x(merge_node_vector[i]);
            merge_node_vector[i]->next_tile_size_y       = calc_next_layer_tile_size_y(merge_node_vector[i]);
            merge_node_vector[i]->branch_input_tile_size_x = calc_branch_next_layer_tile_size_x(merge_node_vector[i]);
            merge_node_vector[i]->branch_input_tile_size_y = calc_branch_next_layer_tile_size_y(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_size_x       = (merge_node_vector[i]->Have_Upsample) ? merge_node_vector[i]->Upsample_size * calc_output_layer_tile_size_x(merge_node_vector[i]) : calc_output_layer_tile_size_x(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_size_y       = (merge_node_vector[i]->Have_Upsample) ? merge_node_vector[i]->Upsample_size * calc_output_layer_tile_size_y(merge_node_vector[i]) : calc_output_layer_tile_size_y(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_number_x   = calc_output_tile_number_x(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_number_y   = calc_output_tile_number_y(merge_node_vector[i]);
            const size_t layer_input_ch                  = std::ceil(merge_node_vector[i]->input_channel/(double)Depth_select)*Depth_select;
            const size_t output_size                     = (merge_node_vector[i]->input_tile_number_x * merge_node_vector[i]->input_tile_number_y)*merge_node_vector[i]->input_tile_size_x*merge_node_vector[i]->input_tile_size_y*layer_input_ch*DATA_WIDTH;
            merge_node_vector[i]->input_address.push_back(0);
            if(merge_node_vector[i]->Have_Maxpooling){
            ...
            std::cout<<merge_node_vector[i]->pool_address<<" "<<merge_node_vector[i]->output_address<<std::endl;
        ...
        else if(merge_node_vector[i]->Have_Concat){
            ...
            const size_t layer_input_ch                = std::ceil(merge_node_vector[i]->input_channel/(double)Depth_select)*Depth_select;
            const size_t output_size                   = (merge_node_vector[i]->input_tile_number_x*merge_node_vector[i]->input_tile_number_y)*merge_node_vector[merge_node_jump_location_select]->input_tile_size_x*merge_node_vector[merge_node_jump_location_select]->input_tile_size_y*layer_input_ch*DATA_WIDTH;
            ...
            if(merge_node_vector[i]->Concat_output_control && merge_node_vector[i]->Have_Maxpooling){
                ...
                auto pool_address_calc = (merge_node_vector[i]->input_tile_number_x * merge_node_vector[i]->input_tile_number_y)*merge_node_vector[i]->input_tile_size_x*merge_node_vector[i]->input_tile_size_y*layer_output_ch*DATA_WIDTH;
        ...
        else if(merge_node_vector[i]->Have_Dense){//為啥只會有x，因為他是全連接只會有一個
            ...
            merge_node_vector[i]->output_padding_size        = merge_node_vector[i]->padding=="same" ? merge_node_vector[i]->input_padding_size : 0;
            merge_node_vector[i]->input_tile_number_dense    = calc_dense_input_tile_number(merge_node_vector[i]);
            merge_node_vector[i]->input_tile_size_x          = calc_dense_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->input_tile_size_y          = calc_dense_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->next_tile_size_x           = calc_dense_next_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->next_tile_size_y           = calc_dense_next_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->branch_input_tile_size_x   = calc_dense_branch_next_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->branch_input_tile_size_y   = calc_dense_branch_next_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_size_x         = (merge_node_vector[i]->Have_Upsample) ? merge_node_vector[i]->Upsample_size * calc_dense_output_layer_tile_size(merge_node_vector[i]) : calc_dense_output_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_size_y         = (merge_node_vector[i]->Have_Upsample) ? merge_node_vector[i]->Upsample_size * calc_dense_output_layer_tile_size(merge_node_vector[i]) : calc_dense_output_layer_tile_size(merge_node_vector[i]);
            merge_node_vector[i]->output_tile_number_dense   = calc_dense_output_tile_number(merge_node_vector[i]);
            const size_t output_size                         = /*這邊的input_tile_size包含了全部的資料*/merge_node_vector[i]->input_tile_number_dense * merge_node_vector[i]->input_tile_size_x * merge_node_vector[i]->input_tile_size_y * DATA_WIDTH;
            ...
            const size_t layer_input_ch                    = std::ceil(merge_node_vector[i]->input_channel/(double)Depth_select)*Depth_select;
            const size_t output_size                       = (merge_node_vector[i]->input_tile_number_x * merge_node_vector[i]->input_tile_number_y )*merge_node_vector[i]->input_tile_size_x*merge_node_vector[i]->input_tile_size_y*layer_input_ch*DATA_WIDTH;
            std::cout<<i<<" "<< "output_size:      "<<output_size<<" "<<merge_node_vector[i]->input_tile_size_x<<" "<<merge_node_vector[i]->input_tile_size_y<<" "<<layer_input_ch<<" "<<" "<<Depth_select<<" "<<merge_node_vector[i]->kernel_size<<std::endl;
            size_t address_select,branch_output_address_select;
            if(!merge_node_vector[i]->branch_node){
                ...
                merge_node_vector[i]->input_address.push_back(address_select/*offset_vector[std::stoi(tmp)-2]*/);//this 2 is preset the count number
                if(merge_node_vector[i]->Concat_output_control && merge_node_vector[i]->Have_Maxpooling){
                    auto layer_output_ch   = std::ceil(merge_node_vector[i]->output_channel/(double)Depth_select)*Depth_select;
                    auto pool_address_calc = (merge_node_vector[i]->input_tile_number_x * merge_node_vector[i]->input_tile_number_y) * merge_node_vector[i]->input_tile_size_x * merge_node_vector[i]->input_tile_size_y *layer_output_ch*DATA_WIDTH;
                    merge_node_vector[i]->output_address     = merge_node_vector[i]->input_address[0] + ((merge_node_vector[i]->Have_Upsample) ? std::pow(merge_node_vector[i]->Upsample_size,2)*output_size : output_size);
                ...
                if(merge_node_vector[i]->Concat_output_control && merge_node_vector[i]->Have_Maxpooling){
                    auto layer_output_ch                = std::ceil(merge_node_vector[i]->output_channel/(double)Depth_select)*Depth_select;
                    auto pool_address_calc = (merge_node_vector[i]->input_feature_size_x * merge_node_vector[i]->input_feature_size_y)*merge_node_vector[i]->input_tile_size_x * merge_node_vector[i]->input_tile_size_y *layer_output_ch*DATA_WIDTH;
                    merge_node_vector[i]->output_address     = branch_output_address_select + ((merge_node_vector[i]->Have_Upsample) ? std::pow(merge_node_vector[i]->Upsample_size,2)*output_size : output_size);
                    merge_node_vector[i]->pool_address       = branch_output_address_select + ((merge_node_vector[i]->Have_Upsample) ? std::pow(merge_node_vector[i]->Upsample_size,2)*pool_address_calc : pool_address_calc);
                }
                else if(merge_node_vector[i]->Have_Maxpooling){
...
size_t ten_valid_type(std::shared_ptr<pstruct> &merge_node,size_t &i,size_t &j){ //for valid address is different with gen_t_type_m
    auto tile_number_x = merge_node->input_tile_number_x; //8
    auto tile_number_y = merge_node->input_tile_number_y; //8
    size_t value;
    if(tile_number_x==1&&tile_number_y==1&&merge_node->input_padding_size!=0)value = 9;
    else if(tile_number_x==1&&tile_number_y==1&&merge_node->input_padding_size==0)value = 10;
	else if(i==0&&j==0)value = 0;
	else if(i==0 && j!=tile_number_x-1)value = 1;
	else if(i==0 && j==tile_number_x-1)value = 2;
	else if(i!=0 && i<tile_number_y-1 && j==0)value = 3;
	else if(i!=0 && i<tile_number_y-1 && j!=tile_number_x-1)value = 4;
	else if(i!=0 && i<tile_number_y-1 && i!=tile_number_y-1 && j==tile_number_x-1)value = 5;
	else if(i==tile_number_y-1 && j==0)value = 6;
	else if(i==tile_number_y-1 && j!=tile_number_x-1)value = 7;
	else value = 8;
    return value;
}
void pstruct::gen_out_addr(std::shared_ptr<pstruct> &merge_node){//input_tile_number output_tile_number 要修改這個兩邊的東西
    const size_t next_tile_size_x = merge_node->next_tile_size_x;
    const size_t next_tile_size_y = merge_node->next_tile_size_y;
    const size_t o_tile_size_x = merge_node->output_tile_size_x;
    const size_t o_tile_size_y = merge_node->output_tile_size_y;
    const size_t input_tile_size_x = merge_node->input_tile_size_x;
    const size_t input_tile_size_y = merge_node->input_tile_size_y;
    const size_t branch_input_tile_size_x = merge_node->branch_input_tile_size_x;
    const size_t branch_input_tile_size_y = merge_node->branch_input_tile_size_y;
    const size_t i_tile_num_x = merge_node->input_tile_number_x;
    const size_t i_tile_num_y = merge_node->input_tile_number_y;
    const size_t o_tile_count_x = i_tile_num_x / merge_node->output_tile_number_x;
    const size_t o_tile_count_y = i_tile_num_y / merge_node->output_tile_number_y;
    const size_t next_input_tile_num_x = merge_node->output_tile_number_x;
    const size_t next_input_tile_num_y = merge_node->output_tile_number_y;
    ...
    if(merge_node->class_name!="Dense"){
        std::cout<<"i_tile_num_x          "<<i_tile_num_x<<std::endl;
        std::cout<<"i_tile_num_y          "<<i_tile_num_y<<std::endl;
        std::cout<<"next_input_tile_num_x "<<next_input_tile_num_x<<std::endl;
        std::cout<<"next_input_tile_num_y "<<next_input_tile_num_y<<std::endl;
    }else{
        std::cout<<"input_tile_number_dense  "<<merge_node->input_tile_number_dense<<std::endl;   
        std::cout<<"output_tile_number_dense "<<merge_node->output_tile_number_dense<<std::endl;
    }
    std::cout<<"next_tile_size_x      "<<next_tile_size_x<<std::endl;
    std::cout<<"next_tile_size_y      "<<next_tile_size_y<<std::endl;
    std::cout<<"o_tile_count_x        "<<o_tile_count_x<<std::endl;
    std::cout<<"o_tile_count_y        "<<o_tile_count_y<<std::endl;
    std::cout<<"o_tile_size_x         "<<o_tile_size_x<<std::endl;
    std::cout<<"o_tile_size_y         "<<o_tile_size_y<<std::endl;
    std::cout<<"channel_align         "<<channel_align<<std::endl;
    std::cout<<"output_address        "<<merge_node->output_address<<std::endl;
    std::cout<<"pool_address          "<<merge_node->pool_address<<std::endl;
    std::cout<<"Depth_select          "<<Depth_select<<std::endl;
    std::cout<<std::endl;
    if(merge_node->class_name!="Dense" && merge_node->class_name!="Flatten" && merge_node->class_name!="LSTM"){
        merge_node->out_addr_set.resize(i_tile_num_x*i_tile_num_y*channel_align,0);
        merge_node->out_addr_set_pool.resize(i_tile_num_x*i_tile_num_y*channel_align,0);
        merge_node->valid_address_vec.resize(i_tile_num_x*i_tile_num_y*channel_align,0);
        size_t tmp,tmp_pool;
        for(size_t i = 0; i < i_tile_num_y; i++){
            if(merge_node->Have_Maxpooling && merge_node->Concat_output_control){
                tmp      =  merge_node->output_address + (i/(i_tile_num_y)) * branch_input_tile_size_x * branch_input_tile_size_y * i_tile_num_x * two_byte_per_ch;//8ch per data 2 byte
                tmp      += (i % i_tile_num_y) * branch_input_tile_size_x * branch_input_tile_size_y * i_tile_num_x * two_byte_per_ch; //add one tiles next_input_tile_number because you got the same line;
                tmp_pool =  merge_node->pool_address + (i/(o_tile_count_y)) * next_tile_size_x * next_tile_size_y * next_input_tile_num_x * two_byte_per_ch;//8ch per data 2 byte
                tmp_pool += (i % o_tile_count_y) * o_tile_size_y * next_tile_size_x * next_input_tile_num_x * two_byte_per_ch; //add one tiles next_input_tile_number because you got the same line;
            }
                ...
                tmp      =  merge_node->output_address + (i/(o_tile_count_y)) * next_tile_size_x * next_tile_size_y * next_input_tile_num_x * two_byte_per_ch;//8ch per data 2 byte
                tmp      += (i % o_tile_count_y) * o_tile_size_y * next_tile_size_x * next_input_tile_num_x * two_byte_per_ch; //add one tiles next_input_tile_number because you got the same line;
            }
            for(size_t j = 0; j < i_tile_num_x; j++){
                for(size_t och = 0; och < channel_align; och++){
                    size_t index = (i*i_tile_num_x+j)*channel_align+och;
                    if(merge_node->Have_Maxpooling){
                        merge_node->out_addr_set_pool[index] = tmp_pool;
                        merge_node->out_addr_set_pool[index] += (j/o_tile_count_x) * next_tile_size_x *  two_byte_per_ch; //sub one times next_tile_size because you get the same line;
                        merge_node->out_addr_set_pool[index] += (j%o_tile_count_x) * o_tile_size_x *  two_byte_per_ch;
                        merge_node->out_addr_set_pool[index] += och * next_tile_size_x * next_tile_size_y * next_input_tile_num_x * next_input_tile_num_y * two_byte_per_ch;
                    }
                    if(merge_node->Concat_output_control || !merge_node->Have_Maxpooling){
                        merge_node->out_addr_set[index] = tmp;
                        merge_node->out_addr_set[index] += (j/i_tile_num_x) *  branch_input_tile_size_x *  two_byte_per_ch; //sub one times next_tile_size because you get the same line;
                        merge_node->out_addr_set[index] += (j%i_tile_num_x) *  branch_input_tile_size_x *  two_byte_per_ch;
                        merge_node->out_addr_set[index] += och * branch_input_tile_size_x * branch_input_tile_size_y * i_tile_num_x * i_tile_num_y * two_byte_per_ch;
                    }
                    ...
                    auto input_tile_size_x = merge_node->input_tile_size_x;
                    auto input_tile_size_y = merge_node->input_tile_size_y;
                    auto input_tile_number_x = merge_node->input_tile_number_x;
                    auto input_tile_number_y = merge_node->input_tile_number_y;
                    if(tile_type==0 || tile_type==9 || tile_type==10){
                        ...
                        merge_node->valid_address_vec[index] = input_address 
                                                             + ((input_tile_size_x*j)-1)*two_byte_per_ch ;
                        ...
                        merge_node->valid_address_vec[index] = input_address 
                                                             + input_tile_size_x * input_tile_size_y *input_tile_number_y*(i-1)*two_byte_per_ch 
                                                             + input_tile_size_x*(input_tile_size_y-1)*input_tile_number_x*two_byte_per_ch;
                    }else if(tile_type==4 || tile_type==5 || tile_type== 7 || tile_type==8){
                        merge_node->valid_address_vec[index] = input_address 
                                                             + ((i-1)*input_tile_size_x * input_tile_size_y *input_tile_number_y 
                                                             + input_tile_size_x*(input_tile_size_y-1)*input_tile_number_x 
                                                             + (input_tile_size_x*j)-1)*two_byte_per_ch;
                    }else{
        ...
        size_t i_tile_number_dense = merge_node->input_tile_number_dense; // in_x * in_y * in_ch
        size_t next_tile_num_dense = merge_node->output_tile_number_dense;// o_x * o_y * o_ch 
        ...
        merge_node->out_addr_set.resize(round_ch_24(i_tile_number_dense)*round_ch_24(next_tile_num_dense),0);
        merge_node->valid_address_vec.resize(round_ch_24(i_tile_number_dense)*round_ch_24(next_tile_num_dense),0);
    ...
    }else if(merge_node->class_name=="LSTM"){//TODO LSTM func
        //first step forget gate complete
        size_t total_input_units = merge_node->input_feature_size_x; 
        size_t gate_units = merge_node->units;
        size_t gate_units_rd = round_ch_8(gate_units);
        size_t input_address = 0;
        merge_node->out_addr_set.resize(total_input_units * gate_units_rd,0);//31 unit * 128/8 = 31*16
        merge_node->valid_address_vec.resize(total_input_units * gate_units_rd,0);//31 unit * 128/8 = 31*16
        for(auto input_unit=0;input_unit<total_input_units;input_unit++){ // total input units 31
            for(auto i=0;i<gate_units_rd;i++){ // how many round in there #16
                size_t index = input_unit*gate_units_rd + i;
                merge_node->out_addr_set[index] = merge_node->output_address + index * two_byte_per_ch;// index * 8 * 2
                merge_node->valid_address_vec[index] = input_address + input_unit * DATA_WIDTH;
            }
        }
    }else{
        std::cout<<"sorry this node cannot support node class is "<<merge_node->class_name<<std::endl;
        throw std::invalid_argument("sorry this node cannot support now!!!");
    }
}
void pstruct::gen_t_type_m(std::shared_ptr<pstruct> &merge_node){
    ...
    if(merge_node->class_name=="LSTM"){
        merge_node->tile_padding_type.resize(merge_node->out_addr_set.size(),11);
        return;
    }
    const size_t i_tile_num_x = merge_node->input_tile_number_x;
    const size_t i_tile_num_y = merge_node->input_tile_number_y;
    for(size_t i = 0;i<i_tile_num_y;i++){
        for(size_t j = 0;j<i_tile_num_x;j++){
            ten_type = 0b0000;
            if(merge_node->kernel_size==2)ten_type = 15;//because hw cannot support kernel size 2, so we need padding in hw by type 15
            else if(merge_node->input_tile_number_x==1&&merge_node->input_tile_number_y==1&&merge_node->input_padding_size!=0)ten_type = 9;
            else if(merge_node->input_tile_number_x==1&&merge_node->input_tile_number_y==1&&merge_node->input_padding_size==0)ten_type = 10;
            else if(merge_node->input_tile_number_x!=0 && merge_node->input_tile_number_y==1 && i==0 && j==0){
                ten_type = 12;
            }
            else if(merge_node->input_tile_number_x!=0 && merge_node->input_tile_number_y==1 && i==0 && j!=merge_node->input_tile_number_x-1){
                ten_type = 13;
            }
            else if(merge_node->input_tile_number_x!=0 && merge_node->input_tile_number_y==1 && i==0 && j==merge_node->input_tile_number_x-1){
                ten_type = 14;
                ...
                ten_type = 0;
            } else if(i==0 && j!=i_tile_num_x-1){
                ten_type = 1;
            } else if(i==0 && j==i_tile_num_x-1){
                ten_type = 2;
            } else if(i!=0 && i<i_tile_num_y-1 &&j==0){
                ten_type = 3;
            } else if(i!=0 && i<i_tile_num_y-1 &&j!=i_tile_num_x-1){
                ten_type = 4;
            } else if(i!=0 && i<i_tile_num_y-1 && i!=i_tile_num_y-1 && j==i_tile_num_x-1){
                ten_type = 5;
            } else if(i==i_tile_num_y-1 && j==0){
                ten_type = 6;
            } else if(i==i_tile_num_y-1 && j!=i_tile_num_x-1){
                ten_type = 7;
            } else if(i==i_tile_num_y-1 && j==i_tile_num_x-1){
                ten_type = 8;
...
void pstruct::get_tile_info(std::vector<std::shared_ptr<pstruct>> &merge_node_vector){
    for(auto merge_node : merge_node_vector){
        ...
        const size_t i_tile_num_x  = merge_node->input_tile_number_x;
        const size_t i_tile_num_y  = merge_node->input_tile_number_y;
        std::cout<<merge_node->class_name<<std::endl;
        if(merge_node->class_name!="Dense" && merge_node->class_name!="LSTM"){
            ...
            merge_node->Tile_Info_Number = i_tile_num_x * i_tile_num_y * ICH_Round * OCH_Round; 
            const size_t tile_info_num  = merge_node->Tile_Info_Number;
            merge_node->tile_info.resize(tile_info_num*8,0);
            const size_t input_tile_number_x = merge_node->input_tile_number_x; //4*4*16
            const size_t input_tile_number_y = merge_node->input_tile_number_y; //4*4*16
            ...
            const size_t HW_KERNEL_SIZE = merge_node->kernel_size==1 ? 1 : 9;
            for(size_t i=0;i<input_tile_number_x*input_tile_number_y;i++){//x y z input_feature_size_x input_feature_size_y och_rd 
                for(size_t j=0;j<OCH_Round;j++){//conv ok flatten fc_x fc_y och_rd but always 1
                    ...
                    for(const size_t layer_ich : merge_node->Previous_node_OCH){
                        ...
                        for(size_t k = 0;k<std::ceil(layer_ich/(double)Depth_select);k++){
                            ...
                            tmp_in_addr += k * merge_node->input_tile_number_x * merge_node->input_tile_number_y * merge_node->input_tile_size_x * merge_node->input_tile_size_y * Depth_select * 2;
                            merge_node->tile_info[index*8+2] = tmp_in_addr;
                            merge_node->tile_info[index*8+3] = merge_node->out_addr_set_pool[i*OCH_Round+j];//pooling_addr fix the address if the tile need to concat
                            ...
                            if((i == input_tile_number_x*input_tile_number_y-1) && (j == OCH_Round-1) && (ich_count == ICH_Round-1))//Is_Final_tile
                            ...
                            merge_node->tile_info[index*8+4] |= (merge_node->input_tile_size_x&0b111111)<<7;         //input_tile_size_row
                            merge_node->tile_info[index*8+4] |= (merge_node->input_tile_size_y&0b111111)<<13;        //input_tile_size_col
                            ...
                            if((ich_count == ICH_Round-1) && (merge_node->Have_BatchNormalization || merge_node->Have_bias)){
                                merge_node->tile_info[index*8+4] |= (merge_node->kernel_size==1) ? (weight_len_last_1&0b1111111)<<19 : (weight_len_last_3&0b1111111)<<19;       //weight_len 74
                                //merge_node->tile_info[index*8] = (merge_node->kernel_size==3) ? (weight_len_last_3&0b1111111) : (weight_len_last_1&0b1111111);       //weight_len 74 debug
                            }
                            else{
                                merge_node->tile_info[index*8+4] |= (weight_len_not_last&0b1111111)<<19;       //weight_len 72
                                //merge_node->tile_info[index*8] = (merge_node->kernel_size==3) ? (weight_len_not_last&0b1111111) : (weight_len_not_last&0b1111111);       //weight_len 74 debug
                            }
                            ...
                            //size_t Output_tile_size_with_out_pooling_x   =  merge_node->input_tile_size_x / (merge_node->kernel_stride_x!=0 ? merge_node->kernel_stride_x : 1); 
                            //size_t Output_tile_size_with_out_pooling_y   =  merge_node->input_tile_size_y / (merge_node->kernel_stride_y!=0 ? merge_node->kernel_stride_y : 1); 
                            size_t Output_tile_size_with_out_pooling_x   =  merge_node->input_tile_size_x + (merge_node->input_padding_size*2) - merge_node->kernel_size + 1; 
                            size_t Output_tile_size_with_out_pooling_y   =  merge_node->input_tile_size_y + (merge_node->input_padding_size*2) - merge_node->kernel_size + 1; 
                            merge_node->tile_info[index*8+4] |= (Output_tile_size_with_out_pooling_x&0b000001)<<31;          //output_tile_size_row
                            merge_node->tile_info[index*8+5]  = (Output_tile_size_with_out_pooling_x&0b111110)>>1;           //output_tile_size_row
                            merge_node->tile_info[index*8+5] |= (Output_tile_size_with_out_pooling_y&0b111111)<<5;           //output_tile_size_col
                            //5bit control
            ...
            size_t i_tile_number_dense = merge_node->input_tile_number_dense; // in_x * in_y * in_ch
            size_t next_tile_num_dense = merge_node->output_tile_number_dense;// o_x * o_y * o_ch 
            merge_node->tile_info.resize(round_ch_24(i_tile_number_dense)*round_ch_24(next_tile_num_dense)*8,0);
        ...
        }else if(merge_node->class_name=="LSTM"){ //TODO LSTM get_tile_info func
            size_t total_input_units = merge_node->input_feature_size_x; //31
            size_t gate_units = merge_node->units;//128
            size_t gate_units_rd = round_ch_8(gate_units);//16
            size_t lstm_gates = 4;
            size_t lstm_bias = 1;
            size_t tile_info_num = total_input_units * gate_units_rd * lstm_gates;
            merge_node->Tile_Info_Number = tile_info_num;
            merge_node->tile_info.resize(tile_info_num*8,0);//total 31cell*16times*4gate tile
            size_t lstm_input_times = round_ch_8(merge_node->input_channel);
            int32_t lstm_input_weight_row,lstm_input_weight_col;
            int32_t lstm_output_weight_row,lstm_output_weight_col;
            lstm_dicd(merge_node->input_channel+merge_node->output_channel,lstm_input_weight_col,lstm_input_weight_row);
            lstm_dicd(merge_node->output_channel,lstm_output_weight_col,lstm_output_weight_row);
            std::cout<<lstm_input_weight_row<<" "<<lstm_input_weight_col<<std::endl;
            for(size_t input_unit=0;input_unit<total_input_units;input_unit++){ // total input units 31
                for(size_t rd=0;rd<gate_units_rd;rd++){ // how many round in there #16
                    for(size_t gate_count=0;gate_count<lstm_gates;gate_count++){ //4 gates
                        size_t index = input_unit*gate_units_rd *lstm_gates+ rd*lstm_gates + gate_count;

                        //---------------output_addr---------------
                        merge_node->tile_info[index*8] = merge_node->out_addr_set[input_unit * gate_units_rd + rd];  //lstm 4 gate output the same location

                        //---------------weight_addr---------------
                        merge_node->tile_info[index*8+1] =  merge_node->weight_address; 
                        merge_node->tile_info[index*8+1] += (input_unit*gate_units_rd+rd) * (merge_node->input_channel + merge_node->output_channel + lstm_bias) * DATA_DEPTH  * lstm_gates * DATA_WIDTH;//one round you need to use (512+128+1)*8*4gate*2byte
                        merge_node->tile_info[index*8+1] += gate_count * (merge_node->input_channel + merge_node->output_channel + lstm_bias) * DATA_DEPTH * DATA_WIDTH; //only one gate is (512+128+1) * 8 * 1gate * 2byte

                        //---------------input_addr---------------
                        merge_node->tile_info[index*8+2] = merge_node->input_address[0];
                        merge_node->tile_info[index*8+2] = merge_node->tile_info[index*8+2] + gate_count * (merge_node->input_tile_size_x * merge_node->input_tile_size_y * DATA_DEPTH * gate_units_rd * DATA_WIDTH); // one complete round 31*1*8*16*2
                        merge_node->tile_info[index*8+2] = merge_node->tile_info[index*8+2] + rd * (merge_node->input_tile_size_x * merge_node->input_tile_size_y * DATA_DEPTH * DATA_WIDTH); //next round 31*1*8*2

                        //---------------pool__addr---------------
                        merge_node->tile_info[index*8+3] = 0; //LSTM Dont have any pool addr you can use this to debug your answer~~
                        //printf("%ld",gate_count);

                        //---------------is_last_channel---------------
                        merge_node->tile_info[index*8+4] = (gate_count==lstm_gates-1) ? 1 : 0; //告訴硬體說這邊是最後一個channel要記得加上alpga bias

                        //---------------have_accumulate---------------
                        merge_node->tile_info[index*8+4] |= 0<<1;

                        //---------------is_final_tile---------------
                        merge_node->tile_info[index*8+4] |= (rd==gate_units_rd-1 && gate_count==lstm_gates-1) ? 1<<2 : 0<<2; //告訴硬體說我這邊要輸出拉因為已經是最後一個了

                        //---------------tile_padding_type---------------
                        merge_node->tile_info[index*8+4] |= (merge_node->tile_padding_type[rd]&0xf) << 3;

                        //---------------Input_Tile_Size_row--------------- but in lstm this is for weight_x
                        merge_node->tile_info[index*8+4] |= (lstm_input_weight_row&0b111111)<<7;

                        //---------------Input_Tile_Size_col--------------- but in lstm this is for weight_y
                        merge_node->tile_info[index*8+4] |= (lstm_input_weight_col&0b111111)<<13;

                        //---------------Weight_len--------------- but in lstm this is for input
                        merge_node->tile_info[index*8+4] |= (lstm_input_times&0b1111111)<<19;       //weight_len 74

                        //---------------Have_Maxpooling----------
                        merge_node->tile_info[index*8+4] |= (merge_node->Have_Maxpooling) ? 1<<26 : 0<<26;

                        //---------------hw_icp_able_cacl---------
                        merge_node->tile_info[index*8+4] |= (1&0b11)<<27;

                        //---------------hw_ocp_able_cacl---------
                        merge_node->tile_info[index*8+4] |= (1&0b11)<<29;

                        //---------------output_Tile_Size_row---------
                        merge_node->tile_info[index*8+4] |= (lstm_output_weight_row&0b000001)<<31;          //output_tile_size_row
                        merge_node->tile_info[index*8+5]  = (lstm_output_weight_row&0b111110)>>1;           //output_tile_size_row

                        //---------------output_Tile_Size_col---------
                        merge_node->tile_info[index*8+5] |= (lstm_output_weight_col&0b111111)<<5;           //output_tile_size_col
            ...
            //U CAN ADD FUNC IN HERE
        }
    }
}
std::vector<uint32_t> pstruct::Gen_Layer_Info(std::shared_ptr<pstruct> &merge_node){ //TODO there need change x y
    ...
    Inst[0] |= (merge_node->pool_stride_x&0b11)<<8;
    Inst[0] |= (merge_node->pool_size_x&0b11)<<10;
    Inst[0] |= (merge_node->kernel_stride_x&0b11)<<12;
    Inst[0] |= (merge_node->kernel_size&0b11)<<14;
    //quant_serias
    Inst[0] |= (merge_node->quant_batch_bias_alpha%0b111111)<<16;
    ...
    Inst[2] |= (merge_node->Have_Upsample&0x1) << 20;//need to fix unsample 
    Inst[2] |= (merge_node->Leaky_ReLU_alpha_FP&0x07ff) << 21;
    Inst[3]  = (merge_node->Leaky_ReLU_alpha_FP&0xf800) >> 11;
    int32_t input_feature_offset_select = merge_node->kernel_size==1 ? merge_node->input_feature_size_x * merge_node->input_feature_size_y : merge_node->input_feature_size_x;//如果是3*3的話，要給予整個input_feature_size，但是如果是1*1那要給予這個tile的offset，例如這個tile是8*8那這個input_feature_offset就是64，最大限制是34*12
    int32_t input_feature_offset;
    if(merge_node->class_name=="Dense"){
        input_feature_offset = merge_node->Dense_size_input_x * merge_node->Dense_size_input_y;
    }else if(merge_node->class_name=="LSTM"){
        input_feature_offset = merge_node->input_channel + merge_node->output_channel;
    }else{
        input_feature_offset = input_feature_offset_select;
    }
    Inst[3] |= (input_feature_offset&0xfff)<<5;
    int32_t output_feature_select_x = merge_node->Have_Maxpooling ? merge_node->output_feature_size_x*2 : merge_node->output_feature_size_x; //這邊幫忙回復到沒有做maxpooling的參數 但是裡面運算還是要有喔~~~
    int32_t output_feature_select_y = merge_node->Have_Maxpooling ? merge_node->output_feature_size_y*2 : merge_node->output_feature_size_y; //這邊幫忙回復到沒有做maxpooling的參數 但是裡面運算還是要有喔~~~
    int32_t output_feature_offset;
    if(merge_node->class_name=="Dense"){
        output_feature_offset = merge_node->Dense_size_output_x * merge_node->Dense_size_output_y;
    }else if(merge_node->class_name=="LSTM"){
        output_feature_offset = merge_node->output_channel;
    }else if(merge_node->kernel_size==1){
        output_feature_offset = output_feature_select_x * output_feature_select_y;
    }else{
        output_feature_offset = output_feature_select_x;
    }
    std::cout<<"check!!!!!!!!!!!!!!!!!!your output_offset "<<output_feature_offset<<std::endl;
    ...
    Inst[4] |= (merge_node->quant_batch_bias_beta&0b111111)<<7;
    std::cout<<merge_node->pooling_quant_finish<<"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
    return Inst;
}
std::vector<uint32_t> pstruct::gen_layer_info_data(std::vector<std::shared_ptr<pstruct>> &merge_node_vector){
    ...
    for(auto &merge_node : merge_node_vector){
        const size_t Depth_select = merge_node->class_name=="Dense" ?  DATA_Depth_24 : merge_node->kernel_size==1 ? DATA_Depth_24 : DATA_DEPTH;
        const size_t tile_num = merge_node->Tile_Info_Number * FOUR_K_BOUNDARY_ZCU_OR_ZEDBOARD_NUMBER; // 1 tile = 256bit but 1 transport is 128 bit , so you need * 2
        ...
        for(size_t j=0;j<tile_num;j+=tile_num_tmp){
            ...
            merge_node->Tile_Info_Number = tile_num_tmp -1;//AXI CH 1024 you need -1 = 1023
            ...
            tmp_addr +=tile_num_tmp * BUS_WIDTH; //1 times transport 128 bit = 16byte
        }
    }
...
std::vector<short>pstruct::gen_layer_weight(const std::shared_ptr<pstruct> &merge_node,const std::vector<std::string> &path_set){
    ...
    size_t once_cal_weight_num_select = merge_node->kernel_size==1 ? ONCE_CAL_WEIGHT_NUM_1 : ONCE_CAL_WEIGHT_NUM_3;
    ...
    size_t weight_num;
    if(merge_node->class_name=="LSTM"){
        weight_num = (merge_node->input_channel + merge_node->output_channel + 1)*merge_node->output_channel*4;
    }else if(merge_node->class_name!="Dense"){
        weight_num = ((OCH_NUM * ICH_NUM * once_cal_weight_num_select) + (Depth_select*2 * OCH_NUM));
    }else{
        weight_num = ((OCH_NUM * ICH_NUM * once_cal_weight_num_select * merge_node->Dense_size_input_x * merge_node->Dense_size_input_y * merge_node->Dense_size_output_x * merge_node->Dense_size_output_y)
                     + (Depth_select * 2 * OCH_NUM * merge_node->Dense_size_output_x * merge_node->Dense_size_output_y));
    }
    //const size_t HW_KERNEL_SIZE = 9;
    const size_t HW_KERNEL_SIZE = (merge_node->kernel_size==1) ? 1 : 9; //這邊有可能是 1 或是 3 現在可以吃進來了,if kernel size is two then the result is three,because the hw cannot support kernel size 2
    std::vector<std::ifstream> file_set;
    //這邊之後要改因為學長這邊搞錯，應該最多有六份才對是weight的bias
    size_t need_file_num;
    if(merge_node->class_name=="LSTM"){
        need_file_num = 3;
    }else if(merge_node->class_name=="Bidirection"){
        need_file_num = 6;
    }else if(merge_node->Have_BatchNormalization){
        if(merge_node->Have_bias)
            need_file_num = 6;
        else
            need_file_num = 5;
    }else if(!merge_node->Have_BatchNormalization){
    ...
    for(size_t i=0;i<need_file_num;i++){
        ...
        if(merge_node->class_name=="LSTM"){
            if(i==0)
                tmp = merge_node->input_channel * merge_node->output_channel*4;
            else if(i==1)
                tmp = merge_node->output_channel * merge_node->output_channel*4;
            else
                tmp = merge_node->output_channel*4;
        }else if(i==0){
            if(IF_PRE_NODE_IS_LSTM)
                tmp = merge_node->input_channel * merge_node->output_channel;
            else if(merge_node->class_name!="Dense")//CONV
        ...
        auto tmp_get_weight_from_file = get_weight_from_file(file_set[i],tmp);
        auto real_kernel_size = merge_node->input_channel * merge_node->output_channel * 3 * 3;//if this node kernel size is 2*2
        std::vector<float>tmp_trans_float_weight_vector(real_kernel_size,0);
        if(merge_node->kernel_size==2 && tmp!=output_channel){//let data to kernel 3*3 && if tmp = output_channel that is bias can dont trans~~ just let it go
            auto file_index = 0;
            for(auto i=0;i<real_kernel_size;i++){
                if(i%9==2 || i%9==5 || i%9==6 || i%9==7 || i%9==8){
                    tmp_trans_float_weight_vector[i] = 0;
                }else{
                    tmp_trans_float_weight_vector[i] = tmp_get_weight_from_file[file_index];
                    file_index++;
                }
            }
        }else{
            tmp_trans_float_weight_vector = tmp_get_weight_from_file;
        }
        data_in.push_back(tmp_trans_float_weight_vector);
        file_set[i].close();
    }
    if(merge_node->kernel_size==2){
        merge_node->kernel_size = 3;
        std::cout<<"CHANGE YOUR KERNEL_SZIE TO THREE!!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
    }
    switch(need_file_num){
        ...
        case 5:
            ...
            break;
        case 3:
            File_Attributes.lstm_weight_x = 0;
            File_Attributes.lstm_weight_h = 0;
            File_Attributes.lstm_bias = 0;
        case 2:
            File_Attributes.weight   = 0;
            File_Attributes.bias     = 1;
            break;
        case 1:
            ...
            break;
        default:
            throw std::invalid_argument("sorry you need to check in there，your file number is wrong not 6 or 5 or 3 or 2 or 1");
            break;
    }
    ...
    if(merge_node->class_name=="Dense"){ //choose the algorithm for this merge_node
        ...
        std::cout<<"LSTMGOGOGOGOGOGOG"<<std::endl;
        algorithm_for_LSTM(merge_node,data_in,weight_data);
    }
    else{
        std::cout<<"CONVGOGOGOGOGOGOG"<<std::endl;
...
std::vector<std::vector<short>>pstruct::gen_total_weight(const std::string &dir_path,const std::vector<std::shared_ptr<pstruct>> &merge_node_vector){
    ...
    for(auto &merge_node : merge_node_vector){
        ...
        if(!weight_table[merge_node->weight_name]){
            auto once_cal_weight_num_select = (merge_node->kernel_size==1) ? ONCE_CAL_WEIGHT_NUM_1 : ONCE_CAL_WEIGHT_NUM_3;
...
std::vector<std::string>pstruct::gen_weight_path(const std::string &dir_path,const std::shared_ptr<pstruct> &merge_node){
    ...
    if(merge_node->class_name=="LSTM"){
        for(size_t i=0;i<path_num;i++){
            switch(i){
                case 0 : //hWeight
                    tmp = merge_node->weight_name + "_f_x.txt";
                    break;
                case 1 : //xWeight
                    tmp = merge_node->weight_name + "_f_h.txt";
                    break;
                case 2 : //Bias
                    tmp = merge_node->weight_name + "_f_bias.txt";
                    break;
            }
            path_set.push_back(dir_path+"/"+tmp);
        }
    }
    else if(path_num==6){ //代表再訓練那邊全開，六份檔案都要有
        for(size_t i=0;i<path_num;i++){
            switch(i){
                case 0 : //Weight
...
template <typename A> std::ostream & operator<<(std::ostream & os,const std::vector<std::shared_ptr<A>> &vec){
    ...
    for(auto elem : vec){
        ...
        os<<counter<<" "<<"input_feature_size_x    "<<elem->input_feature_size_x<<"\n";
        os<<counter<<" "<<"input_feature_size_y    "<<elem->input_feature_size_y<<"\n";
        os<<counter<<" "<<"output_channel          "<<elem->output_channel<<"\n";
        os<<counter<<" "<<"output_feature_size_x   "<<elem->output_feature_size_x<<"\n";
        os<<counter<<" "<<"output_feature_size_y   "<<elem->output_feature_size_y<<"\n";
        if(elem->kernel_size==2){
        ...
        os<<counter<<" "<<"pool_size_x             "<<elem->pool_size_x<<"\n";
        os<<counter<<" "<<"pool_size_y             "<<elem->pool_size_y<<"\n";
        os<<counter<<" "<<"pool_stride_x           "<<elem->pool_stride_x<<"\n";
        os<<counter<<" "<<"pool_stride_y           "<<elem->pool_stride_y<<"\n";
        ...
        os<<counter<<" "<<"input_tile_number_x     "<<elem->input_tile_number_x<<"\n";
        os<<counter<<" "<<"input_tile_number_y     "<<elem->input_tile_number_y<<"\n";
        os<<counter<<" "<<"input_tile_size_x       "<<elem->input_tile_size_x<<"\n";
        os<<counter<<" "<<"input_tile_size_y       "<<elem->input_tile_size_y<<"\n";
        os<<counter<<" "<<"output_tile_number_x    "<<elem->output_tile_number_x<<"\n";
        os<<counter<<" "<<"output_tile_number_y    "<<elem->output_tile_number_y<<"\n";
        os<<counter<<" "<<"output_tile_size_x      "<<elem->output_tile_size_x<<"\n";
        os<<counter<<" "<<"output_tile_size_y      "<<elem->output_tile_size_y<<"\n";
        os<<counter<<" "<<"next_tile_size_x        "<<elem->next_tile_size_x<<"\n";
        os<<counter<<" "<<"next_tile_size_y        "<<elem->next_tile_size_y<<"\n";
        os<<counter<<" "<<"branch_input_tile_size_x"<<elem->branch_input_tile_size_x<<"\n";
        os<<counter<<" "<<"branch_input_tile_size_y"<<elem->branch_input_tile_size_y<<"\n";
        ...
        os<<counter<<" "<<"quant_batch_bias_alpha  "<<elem->quant_batch_bias_alpha<<"\n";     
        os<<counter<<" "<<"quant_batch_bias_beta   "<<elem->quant_batch_bias_beta<<"\n";     
